...
export function CourseWorkspace({id}:{id:string}) {
  const courseId = id;
  const { data, billing, completeLesson, userNotes, saveNote, enrolledCourses, enrollCourse, lastLessons, api, noteStatus } = useLearning();
  ...
  if(!enrolledCourses.includes(courseId)||(course.access==='pro'&&billing.plan!=='pro'))return <div className="course-entry"><section className={`lesson-stage ${course.art}`}><div className="lesson-stage-top"><span><BookOpen/> {course.category}</span><span>{course.access === 'pro' ? 'Pro course' : 'Free course'}</span></div><div className="lesson-stage-content"><p className="library-eyebrow">{course.level} · {course.duration}</p><h2>{course.title}</h2><p>{course.description}</p></div><div className="lesson-stage-bottom"><span>By {course.instructor}</span><span>{lessons.length} lessons</span></div></section><section className="surface backend-panel course-entry-action"><div><p className="library-eyebrow">READY WHEN YOU ARE</p><h2>{course.access === 'pro' && billing.plan !== 'pro' ? 'Unlock this course with Pro' : 'Start learning today'}</h2><p>Your progress and notes stay on your account if your membership ends.</p></div>{course.access === 'pro' && billing.plan !== 'pro' ? <Link href="/pricing" className="btn btn-primary">View Pro plans</Link> : <button className="btn btn-primary" onClick={()=>void enrollCourse(courseId)}>Enroll in this course</button>}</section></div>;
