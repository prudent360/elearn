...
export function createService(db,seed=[],options={}) {
  ...
  async function seedCourses(){
    // Only course content is seeded. Learner identities, activity, grades, and certificates are never fabricated.
    if(await get('SELECT id FROM courses LIMIT 1'))return;
    const statements=[];
    for(const source of seed){const {modules,...metadata}=source;metadata.progress=0;metadata.saved=false;metadata.status='available';
      ...
      (modules||[]).forEach((module,mi)=>module.lessons.forEach((lesson,li)=>{const content={...lesson,completed:false};delete content.videoUrl;if(content.type==='video')content.type='reading'; // Never treat stock demo clips as teaching material.
  ...
  let ready;
  async function currentUser(request,required=true){const token=readSession(externalRequest(request));const user=token&&await get('SELECT u.* FROM users u JOIN sessions s ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>? AND u.disabled=0',digest(token),Date.now());if(!user&&required)fail(401,'Please sign in.');return user||null}
  ...
  async function snapshot(user){
    ...
    return {user:publicUser(user),courses,savedCourses,enrolledCourses,userNotes:Object.fromEntries(notes.map(n=>[n.lesson_id,n.body])),settings,
  ...
  async function handle(request){
    try {
      ...
      if(!['GET','HEAD'].includes(method)){
        const origin=request.headers.get('origin');const expected=options.origin||url.origin;
      ...
      if(path[0]==='auth'){
        ...
        if(method==='POST'&&['register','login','setup'].includes(path[1])){
          ...
          const password=body.password;if(typeof password!=='string'||password.length<12||password.length>128)fail(400,'Use a password between 12 and 128 characters.');
          ...
          if(path[1]==='login'){
            ...
            user=await get('SELECT * FROM users WHERE id=?',id);if(!user)fail(409,'Administrator setup is already complete.');
          }
      ...
      if(path[0]==='courses'&&path[1]){
        ...
        if(path[2]==='enrollment'&&method==='POST'){if(!(await get('SELECT id FROM lessons WHERE course_id=? LIMIT 1',course.id)))fail(409,'This course has no lessons yet.');await run('INSERT OR IGNORE INTO enrollments VALUES(?,?,?)',user.id,course.id,now());return reply({ok:true})}
      ...
      if(path[0]==='instructor'){
        ...
        if(path[1]==='courses'&&((method==='POST'&&!path[2])||(method==='PUT'&&path[2]))){
          const existing=path[2]?await owned(user,path[2]):null;const id=existing?.id||randomUUID();const metadata=existing?JSON.parse(existing.metadata):{thumbnail:'',art:'violet',symbol:'book-open',rating:0,level:'Beginner',duration:'Self-paced',outcomes:[],requirements:[]};
          for(const key of ['title','description','category','level','duration'])if(body[key]!==undefined)metadata[key]=text(body[key],key,key==='description'?5000:200);
      ...
      if(path[0]==='admin'){
        admin(user);
        if(path[1]==='users'&&method==='GET')return reply((await all('SELECT id,email,name,role,disabled,created_at FROM users ORDER BY created_at DESC')).map(publicUser));
